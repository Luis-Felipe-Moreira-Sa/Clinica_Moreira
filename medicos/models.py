...
class Medico(models.Model):
    ...
    regex=r'^\+?1?\d{9,15}$',
    message="É necessario que o numero esteja com esta formatação: \
                    '+55 55 55555-5555'.")
...
def validar_dia(value):
    ...
    if value < today:
        raise ValidationError('Não é possivel selecionar data passada')
    if (weekday == 5) or (weekday == 6):
        raise ValidationError('Selecione um dia útil')
#Anexe uma data para a agenda
class Agenda(models.Model):
    medico = ForeignKey(Medico, on_delete=models.CASCADE, related_name='agenda')
    dia = models.DateField(help_text="Anexe uma data para a agenda", validators=[validar_dia])
